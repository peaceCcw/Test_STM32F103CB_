...
void rs485_init(uint32_t baudrate)
{
    ...
    HAL_NVIC_SetPriority(RS485_UX_IRQn, 7, 0);            /* 抢占优先级7，子优先级0 */
#endif
 ...
 * @retval      无
 */
void rs485_receive_data(uint8_t *buf, uint8_t *len ,uint8_t t)
{
...
//    delay_ms(10); /* 等待10ms,连续超过10ms没有接收到一个数据,则认为接收结束 */
    ...
    if (rxlen == g_RS485_rx_cnt && rxlen == t ) /* 接收到了数据,且接收完成了 */
    {
        for (i = 0; i < rxlen; i++)
        {
