...
//    if ((uint32_t)ADC_ADCX_DMACx > (uint32_t)DMA1_Channel7)                 /* 大于DMA1_Channel7, 则为DMA2的通道了，zbt没有DMA2 */
//    {
...
//    __HAL_RCC_TIM3_CLK_ENABLE();                                        /* TIM3时钟使能 */
    ...
    tim3_handle.Instance = TIM3;                                        /* 选择定时器3 */
    tim3_handle.Init.CounterMode = TIM_COUNTERMODE_CENTERALIGNED1;      // 中央对齐模式
    
    tim3_handle.Init.Prescaler = psc;                                   /* 分频系数 */
    tim3_handle.Init.CounterMode = TIM_COUNTERMODE_UP;                  /* 向上计数 */
    tim3_handle.Init.Period = arr;                                      /* 重装载值 */
    tim3_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE; /* 自动重装 */
    HAL_TIM_Base_Init(&tim3_handle);                                    /* 初始化定时器3 */

    master_config.MasterOutputTrigger = TIM_TRGO_UPDATE;
    master_config.MasterSlaveMode = TIM_MASTERSLAVEMODE_DISABLE;
    HAL_TIMEx_MasterConfigSynchronization(&tim3_handle, &master_config);/* 配置TIM3 TRGO */

    HAL_TIM_Base_Start(&tim3_handle);                                   /* 使能定时器3 */
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
    ...
    {
        __HAL_RCC_TIM3_CLK_ENABLE();            /*使能TIM时钟*/
        HAL_NVIC_SetPriority(TIM3_IRQn, 6, 0); /* 抢占6，子优先级0，组4 */
...
//    HAL_GPIO_Init(GPIOA , &gpio_init_struct);
    ...
    g_adc_nch_dma_handle.Init.ContinuousConvMode = DISABLE;                    /* 使能连续转换 */
