 ...
 * 比如:g_oled1_gram[0][0],包含了第一列,第1~8行的数据. g_oled1_gram[0][0].7,即表示坐标(0,0)
 * 类似的: g_oled1_gram[1][0].6,表示坐标(1,1), g_oled1_gram[10][1].5,表示坐标(10,10),
 *
 * 存放格式如下(高位表示低行数).
 * [0]0 1 2 3 ... 127
 * [1]0 1 2 3 ... 127
...
void oled1_refresh_gram(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        oled1_wr_byte (0xb0 + i, oled1_CMD);  /* 设置页地址（0~7） */
        oled1_wr_byte (0x00, oled1_CMD);      /* 设置显示位置—列低地址 */
        oled1_wr_byte (0x10, oled1_CMD);      /* 设置显示位置—列高地址 */
...
{
    oled1_wr_byte(0X8D, oled1_CMD);   /* SET DCDC命令 */
...
void oled1_show_num(uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size , uint8_t mode)
{
    ...
    {
        temp = (num / oled1_pow(10, len - t - 1)) % 10;              /* 获取对应位的数字 */
        ...
        if (enshow == 0 && t < (len - 1))                           /* 没有使能显示,且还有位要显示 */
        {
            ...
            {   
                ...
                {
                     oled1_show_char(x + (size / 2)*t, y, '0' , size, 1);    /* 用0占位 */
                }
                else
                {
                    oled1_show_char(x + (size / 2)*t, y, ' ', size, 1);  /* 显示空格,站位 */
                }      
                continue;                                           /* 继续下个一位 */
            }
            else
        ...
        oled1_show_char(x + (size / 2)*t, y, temp + '0', size, 1);    /* 显示字符 */
    }
}
...
void oled1_show_string(uint8_t x, uint8_t y, const char *p, uint8_t size)
{
    while ((*p <= '~') && (*p >= ' '))      /* 判断是不是非法字符! */
    {
        ...
        oled1_show_char(x, y, *p, size, 1);  /* 显示一个字符 */
...
void oled1_ShowChinese(uint8_t x, uint8_t y, uint8_t num)
{
    ...
    // 1. 选择对应汉字的字模
    switch(num) {
        case 0: font_ptr = &Hzk1_1; break; // 角
        case 1: font_ptr = &Hzk1_2; break; // 度
        case 2: font_ptr = &Hzk1_3; break; // 压
        case 3: font_ptr = &Hzk1_4; break; // 力
        case 4: font_ptr = &Hzk1_5; break; // 温
...
void oled1_ScrollDisplay(uint8_t num,uint8_t space)
{
    ...
    while(1)
    {
        if(m==0)
        {
        oled1_ShowChinese(128,24,t); //写入一个汉字保存在oled1_GRAM[][]数组中
            t++;
        }
        if(t==num)
        {
...
void oled1_init(void)
{
    ...
    oled1_wr_byte(0xAE, oled1_CMD);   /* 关闭显示 */
...
void oled1_part_refresh_gram(void)
{
    ...
    for (i = 0; i < 8; i++) // 遍历每一页
    {
        ...
        if (start_col <= end_col)
        {
            ...
            // 设置列地址范围
            oled1_wr_byte(0x00 | (start_col & 0x0F), oled1_CMD);        // 列低地址
            oled1_wr_byte(0x10 | ((start_col >> 4) & 0x0F), oled1_CMD); // 列高地址
